...
// Categories aligned with site architecture and Bauhaus principles
const CATEGORIES = {
  design: {
    terms: [
      'design', 'visual', 'typography', 'type', 'font', 'minimal', 'bauhaus',
      'brand', 'identity', 'art direction', 'composition', 'layout', 'grid',
      'color', 'aesthetic', 'modernist', 'style', 'gallery', 'portfolio'
    ],
    meta: ['design', 'typography', 'visual', 'minimal', 'creative']
  },
  make: {
    terms: [
      'tool', 'product', 'app', 'create', 'build', 'craft', 'generator',
      'studio', 'workshop', 'platform', 'maker', 'prototype', 'experiment'
    ],
    meta: ['tool', 'product', 'maker', 'studio', 'app']
  },
  dev: {
    terms: [
      'astro', 'react', 'vue', 'javascript', 'typescript', 'tailwind',
      'performance', 'api', 'engineering', 'code', 'development', 'web',
      'framework', 'library', 'optimization', 'static site', 'mdx'
    ],
    meta: ['development', 'engineering', 'web', 'programming']
  },
  read: {
    terms: [
      'article', 'blog', 'guide', 'tutorial', 'resource', 'book',
      'documentation', 'reference', 'journal', 'essay', 'writing'
    ],
    meta: ['blog', 'article', 'guide', 'resource', 'writing']
  }
};

// Optimized metadata extraction focused on performance
async function extractMetadata(url, html) {
  try {
    ...
    // Get main content efficiently, prioritizing important sections
    const mainContent = 
      $('main').first().text() || 
      $('article').first().text() || 
      $('.content').first().text() || 
...
function analyzeContent(metadata) {
  ...
  for (const [category, data] of Object.entries(CATEGORIES)) {
    ...
    // Check terms in content with weighted scoring
...
// Generate minimal description following Bauhaus principles
function generateDescription(metadata) {
  const { description, title, source } = metadata;
  
  // If we have a source, prioritize it for attribution
  if (source) {
    return `Via ${source}`;
  }
...
// Analyze URL with optimized rate limiting and error handling
...
// Process bookmarks with optimized batching and rate limiting
