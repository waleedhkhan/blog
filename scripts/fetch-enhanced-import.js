...
// Using built-in fetch API instead of node-fetch package
...
function extractViaInfo(html) {
  ...
  try {
    ...
    if (viaElement.length > 0) {
      ...
      if (viaText.startsWith('Via')) {
        return 'Via: ' + viaText.replace('Via', '').trim();
      }
    }
    return '';
  } catch (error) {
...
async function processJsonFeed(jsonData) {
  ...
  console.log(`Processing ${items.length} bookmarks. This will take some time...`);
  ...
  for (let i = 0; i < items.length; i += BATCH_SIZE) {
    ...
    const batchPromises = batch.map(async (item, index) => {
      try {
        ...
        // Extract the Via information first
        ...
        // If via info exists, use that as the primary description
        if (viaInfo) {
          description = viaInfo;
        } 
        // Otherwise use fetched metadata description
        else if (metadata && metadata.description) {
          description = metadata.description;
        } 
        // Fall back to a simple description
        else {
          description = 'Interesting bookmark';
        }
